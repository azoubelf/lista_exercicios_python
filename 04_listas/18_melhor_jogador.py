...
Uma grande emissora de televisão quer fazer uma enquete entre os
...
digitará um número, entre 1 e 23, correspondente ao número da camisa
do jogador. Um número de jogador igual zero, indica que a votação foi
encerrada. Se um número inválido for digitado, o programa deve ignorá-lo,
...
número de votos de um jogador e o total de votos. A função calculará o percentual
e retornará o valor calculado. Abaixo segue uma tela de exemplo. O disposição
das informações deve ser o mais próxima possível ao exemplo. Os dados são fictícios
e podem mudar a cada execução do programa. Ao final, o programa deve ainda gravar
os dados referentes ao resultado da votação em um arquivo texto no disco, obedecendo
a mesma disposição apresentada na tela.
...
while True:
    voto = int(input('Digite o seu voto [1-23]: '))
    if voto == 0:
        break
    if voto > 23:
        print('Voto inválido!')
...
num_votos = sum(jogadores_votos)

porcentagem_votos = [ (voto * 100) / sum(jogadores_votos) for voto in jogadores_votos ]

jogador = 'Jogador'.ljust(15)
votos = 'Votos'.ljust(15)
porcentagem = '%'.ljust(15)
topo_tabela = f'\n{jogador} {votos} {porcentagem}\n'

print(topo_tabela)

for i in range(1, 24):
    print(f'{str(i).ljust(15)} {str(jogadores_votos[i-1]).ljust(15)} {porcentagem_votos[i-1]:.1f}%')
